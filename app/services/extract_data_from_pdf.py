...
def obtainDataFromTables(pdf_path: str):
    dataframe = []
    try:
        tables = tabula.read_pdf(str(pdf_path), pages="all", multiple_tables=True, lattice=True, pandas_options={'dtype': str})
...
def extract_data_from_pdf(pdf_path: str) -> dict:
    ...
    dataFromTables = obtainDataFromTables(fixed_pdf)
