...
def obtainDataFromTables(pdf_path: str, lattice: bool=True):
    dataframe = []
    try:
        tables = tabula.read_pdf(str(pdf_path), pages="all", multiple_tables=True, lattice=lattice, pandas_options={'dtype': str})
...
def extract_data_from_pdf(pdf_path: str, lattice: bool=True) -> dict:
    print(f"extract_data (lattice={lattice})")

    fixed_pdf = normalize_rotation(pdf_path)

    dataFromTables = obtainDataFromTables(fixed_pdf, lattice)
