...
def extraer_steel_grade_de_mtc(paths_mtc_pdfs: List[str], coladas_a_buscar: List[str]) -> Dict[str, str]:
    ...
    for path_mtc in paths_mtc_pdfs:
        ...
        # Normalizamos rotación una vez
        path_mtc_normalizado = normalize_rotation(path_mtc)
        
        # Estructura optimizada: Guardamos { pagina: { 'coladas': [], 'texto': "..." } }
        ...
        try:
            ...
            for i, page in enumerate(reader.pages):
                ...
                if encontradas_aqui:
                    page_num_actual = i + 1
                    # Guardamos el texto para no tener que volver a leerlo
        ...
        for page_num, info in mapa_pagina_info.items():
            ...
            # Buscamos el Steel Grade una sola vez por página
            steel_grade = _extraer_valor_de_pagina(
                path_mtc_pdf_original=str(path_mtc_normalizado),
                page_num=page_num,
                valor_a_buscar="Steel Grade",
                texto_pagina_cache=texto # <--- Pasamos el texto para la búsqueda rápida
            )
            
            for colada in coladas:
                resultados[colada] = steel_grade

        # Limpieza de archivo temporal si fue creado por normalización
