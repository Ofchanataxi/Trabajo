...
# Importamos la nueva función buscar_coladas_en_pdfs
...
def _proceso_interno(path_tally: str, files_info_mtc: List[tuple], files_info_itp: List[tuple]) -> pd.DataFrame:
    ...
    # 3. Mapear Archivo ITP (NUEVO)
    tally_df['itpFilename'] = tally_df[columna_colada].astype(str).map(
        lambda x: mapa_resultados_itp.get(x, "-")
    )

    # 4. Mapear y Concatenar "Tubing" o "Casing" a la descripción
